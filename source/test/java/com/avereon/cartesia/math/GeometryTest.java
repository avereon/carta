...
public class GeometryTest implements NumericTest {
	...
	void testParsePolarCoordinates() {
		assertThat( Geometry.parsePoint( ">0,1" ), closeTo( new Point3D( 1, 0, 0 ), TOLERANCE ) );
		assertThat( Geometry.parsePoint( ">pi/2,1" ), closeTo( new Point3D( 0, 1, 0 ), TOLERANCE ) );
		assertThat( Geometry.parsePoint( ">pi,1" ), closeTo( new Point3D( -1, 0, 0 ), TOLERANCE ) );
		assertThat( Geometry.parsePoint( ">3*pi/2,1" ), closeTo( new Point3D( 0, -1, 0 ), TOLERANCE ) );

		assertThat( Geometry.parsePoint( ">rad(90),1" ), closeTo( new Point3D( 0, 1, 0 ), TOLERANCE ) );
		assertThat( Geometry.parsePoint( ">rad(180),1" ), closeTo( new Point3D( -1, 0, 0 ), TOLERANCE ) );
		assertThat( Geometry.parsePoint( ">rad(270),1" ), closeTo( new Point3D( 0, -1, 0 ), TOLERANCE ) );

		assertThat( Geometry.parsePoint( ">rad(45),1" ), closeTo( new Point3D( java.lang.Math.cos( PI_OVER_4 ), java.lang.Math.sin( PI_OVER_4 ), 0 ), TOLERANCE ) );
	}

	@Test
	void testParseRelativePolarCoordinates() {
		Point3D anchor = new Point3D( 1, 1, 0 );
		assertThat( Geometry.parsePoint( "@<rad(180+45),1", anchor ), closeTo( new Point3D( 1 - java.lang.Math.cos( PI_OVER_4 ), 1 - java.lang.Math.sin( PI_OVER_4 ), 0 ), TOLERANCE ) );
		assertThat( Geometry.parsePoint( "@>rad(180+45),1", anchor ), closeTo( new Point3D( 1 - java.lang.Math.cos( PI_OVER_4 ), 1 - java.lang.Math.sin( PI_OVER_4 ), 0 ), TOLERANCE ) );
	...
	void testParsePointWithWhitespace() {
		...
		assertThat( Geometry.parsePoint( " @ < rad ( 180 + 45 ) , 1 ", anchor ),
			closeTo( new Point3D( 1 - java.lang.Math.cos( PI_OVER_4 ), 1 - java.lang.Math.sin( PI_OVER_4 ), 0 ), TOLERANCE )
		);
	}
