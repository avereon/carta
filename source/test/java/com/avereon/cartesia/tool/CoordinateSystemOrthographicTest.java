...
public class CoordinateSystemOrthographicTest {
	...
	void testFindNearest() throws Exception {
		...
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( 0.3, 0.2, 0 ) ) ).isCloseTo( Point3D.ZERO );
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( -0.3, 0.2, 0 ) ) ).isCloseTo( Point3D.ZERO );
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( -0.3, -0.2, 0 ) ) ).isCloseTo( Point3D.ZERO );
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( 0.3, -0.2, 0 ) ) ).isCloseTo( Point3D.ZERO );

		workplane.setSnapGridX( "0.2" );
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( -14.623984, 2.34873, 0 ) ) ).isCloseTo( new Point3D( -14.6, 2, 0 ) );
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( -4.623984, -6.34873, 0 ) ) ).isCloseTo( new Point3D( -4.6, -6, 0 ) );
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( 7.623984, -23.34873, 0 ) ) ).isCloseTo( new Point3D( 7.6, -23, 0 ) );
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, new Point3D( 67.623984, 13.34873, 0 ) ) ).isCloseTo( new Point3D( 67.6, 13, 0 ) );
	}

	@Test
	void testFindNearestAtZero() throws Exception {
		DesignWorkplane workplane = new DesignWorkplane();
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, Point3D.ZERO ) ).isEqualTo( Point3D.ZERO );
	}
	...
	void testFindNearestOffsetOrigin() throws Exception {
		...
		PointAssert.assertThat( CoordinateSystem.ORTHO.getNearest( workplane, Point3D.ZERO ) ).isCloseTo( new Point3D( 0.3, 0.2, 0 ) );
