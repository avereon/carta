...
public class DesignPaneMarea extends StackPane {
	...
	public DesignPaneMarea setDesign( Design design ) {
	...
	public List<DesignLayerPane> getLayers() {
		// FIXME return getLayers( layers );
		return List.of();
	}

	public List<DesignLayerPane> getVisibleLayers() {
		// FIXME return getLayers( layers ).stream().filter( DesignLayerPane::isEnabled ).collect( Collectors.toList() );
		return List.of();
	}
...
//		cp.typeProperty().unbind();
		// FIXME reference.getChildren().add( cp.setType( DesignMarker.Type.REFERENCE ) );
	}
